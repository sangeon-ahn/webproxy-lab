...
void doit(int fd)
{
  ...
  if (is_static) {
    ...
    save_static_file(fd, filename, sbuf.st_size);
  }
  else {
    clienterror(fd, filename, "501", "Not implemented", "Tiny does not implement this program");
    return;
  }
}

void save_static_file(int fd, char *filename, int filesize)
{
  int filefd;
  char *filesp; // 파일의 스타팅 포인터
  ...
  filesp = Mmap(0, filesize, PROT_READ, MAP_PRIVATE, filefd, 0);
  Close(filefd);

  // 새파일 만들고
  int newfd = Open("new_file.txt", O_WRONLY | O_CREAT, 0644);

  // 쓰기
  Rio_writen(newfd, filesp, filesize);
  
  // 메모리 해제
  Munmap(filesp, filesize);
}
